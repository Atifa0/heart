...
class MeasurePageView extends State<MeasurePage> {
  ...
  _addToDatabase() async {
    ...
    String _formatDate =
        (DateFormat('yyyy-MM-dd HH:mm:ss').format(_dateTime)).toString();
    Measure measure = Measure(
        result: _score as String,
        date: _formatDate,
        img: 'heart.png',
        graph: _chartImage as Uint8List);
              ...
              child: Padding(
                  ...
                  child:
                    ...
                    _toggled && _timeToStartCounter > 0
                        ? Text('${_timeToStartCounter}s to start')
                        : SizedBox(),

                    // Wykres
                    Expanded(
                        child: RepaintBoundary(
                            key: _chartKey, child: Chart(_data))),

                    //Tekst z odliczaniem
                    _toggled &&
                            (_timerCountdown != null &&
                                _timeToStartCounter <= 0)
                        ? Text(
                            '${((_timerCountdown! / 1000).round()).toString()}s to finish.')
                        : SizedBox(),
